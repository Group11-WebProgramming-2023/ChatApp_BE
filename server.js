...
io.on("connection", async (socket) => {
  ...
  socket.on("start_conversation", async (data) => {
    ...
    // if no => create a new chatMessage doc & emit event "start_chat" & send conversation details as payload
    if (existing_conversations.length === 0) {
      let new_chat = await chatMessage.create({
        participants: [to, from],
      });

      new_chat = await chatMessage
        .findById(new_chat)
        .populate("participants", "firstName lastName _id email status");
  ...
  socket.on("text_message", async (data) => {
    ...
    // fetch chatMessage Doc & push a new message to existing conversation
    const chat = await chatMessage.findById(conversation_id);
